...
if __name__ == "__main__":
    ...
    data = connector.read_file("./adult.csv", file_type=FileType.CSV)
    print(data.head())
    ...
    # Create and export the synthetic data profiling report as html
    report_synth = ProfileReport(
        synth_sample, title="My first Profile Report for Synthetic Data using YData"
    )
    report_synth.to_file("synthetic_profiling.html")

    # Compare the synthetic data with the original data
    comparison_report = report.compare(report_synth)
    comparison_report.to_file(
        "comparison_profiling_report.html"
    )  # This will save the report as a shareable HTML file

    # Write parquet file to your destination
    connector.write_file(
        synth_sample.to_pandas(), path="./data.parquet", file_type=FileType.PARQUET
    )
    ...
    metrics_report = SyntheticDataProfile(
        data[cols], synth_sample[cols], metadata=metadata, data_types=synth.data_types
    )

    # Export the report as a shareable PDF file
    metrics_report.generate_report(
        output_path="./synthetic_data_metrics_report.pdf",
    )
